...
| `RobinhoodStockTokenAdapter` + its `MockChainlinkFeed` reference price | **Deployed but unwired.** The redeployed instance (`0x3A1B…dfb9`) returns a valid `AssetState` when called (see sequencer-feed finding above). Used only once, at deploy-script time, to compute the initial seed value passed into `Registry.initializeAsset` — never read live by Registry, Policy, or any consumer contract afterward. |
