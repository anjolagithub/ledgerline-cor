#!/usr/bin/env bash
# LedgerLine Core — Phase 8 fix: upgrade Node via nvm, fix missing app/admin dir
set -uo pipefail
...
echo "=================================================="
echo "Installing nvm (if not already present) + current LTS Node"
echo "=================================================="
if [ ! -d "$HOME/.nvm" ]; then
...
# Make this permanent for future terminal sessions too
if ! grep -q 'NVM_DIR' "$HOME/.bashrc" 2>/dev/null; then
  ...
  echo "Added nvm loading to ~/.bashrc for future sessions."
fi
